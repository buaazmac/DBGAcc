...
//#define ZMAC_DEBUG
  ...
  stream << "[ZMAC_DEBUG] tread-" << tid << " is handling bucket-" << b << std::endl; 
  SimpleTimer zmac_timer;
  //std::cout << stream.str();
  zmac_timer.reset();
  zmac_timer.start();
#endif
...
#ifdef ZMAC_DEBUG
  zmac_timer.stop();
  stream << "\t\t\tthread-" << tid << " spend " << zmac_timer.elapsed() << " in ExtractSubstr" << std::endl;
  zmac_timer.reset();
  zmac_timer.start();
#endif
...
#ifdef ZMAC_DEBUG
  zmac_timer.stop();
  stream << "\t\t\tthread-" << tid << " spend " << zmac_timer.elapsed() << " in Sorting" << std::endl;
  zmac_timer.reset();
  zmac_timer.start();
#endif
...
#ifdef ZMAC_DEBUG
  zmac_timer.stop();
  stream << "\t\t\tthread-" << tid << " spend " << zmac_timer.elapsed() << " in Writing" << std::endl;
  zmac_timer.reset();
  zmac_timer.start();
  std::cout << stream.str();
#endif
