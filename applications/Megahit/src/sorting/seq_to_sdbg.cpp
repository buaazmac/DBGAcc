...
SeqToSdbg::MemoryStat SeqToSdbg::Initialize() {
  // ZMAC
  std::cout << "[ZMAC] Start Initialize()! ... reading file: " << opt_.input_prefix << std::endl;
...
//#define ZMAC_DEBUG
    ...
    stream << "[ZMAC] trying to copy string from " << reinterpret_cast<std::uintptr_t>(edge_p + offset + start_offset) << " to " << reinterpret_cast<std::uintptr_t>(&(*substr)) << " in bucket-" << fetcher.CurBucket() << std::endl; 
    std::cout << stream.str();
#endif
#undef ZMAC_DEBUG
