...
KmerCounter::MemoryStat KmerCounter::Initialize() {
  ...
  xinfo("{} bases, {} reads, {} max read length\n", num_bases, num_reads, seq_pkg_.max_length());
  ...
  // --- initialize writer ---
  std::cout << "[ZMAC] Initilizing an edge writer: out_prefix: " << opt_.output_prefix << ", n_threds: " << opt_.n_threads << ", k: " << opt_.k << ", kNumB: " << kNumBuckets << std::endl;
  ...
  xinfo("The data requires {} bytes memory\n", memory_for_data);
...
//#define ZMAC_DEBUG
    ...
    stream << "[ZMAC] trying to copy string from " << reinterpret_cast<std::uintptr_t>(read_p + offset + start_offset) << " to " << reinterpret_cast<std::uintptr_t>(&(*substr_ptr)) << " in bucket-" << fetcher.CurBucket() << std::endl; 
    std::cout << stream.str();
#endif
#undef ZMAC_DEBUG
