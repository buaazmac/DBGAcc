...
void KmerCounter::Lv0ReportInfo(std::vector<ThreadMeta> t_meta, unsigned n_threads) {
  int read_start = t_meta[0].seq_from,
      read_end = t_meta[n_threads - 1].seq_to - 1;
  std::cout << t_meta[0].seq_from << ", " << t_meta[0].seq_to << ", " 
            << t_meta[n_threads - 1].seq_from << ", " << t_meta[n_threads - 1].seq_to - 1
            << ", " << seq_pkg_.seq_count()<< std::endl; 
  auto seq_start = seq_pkg_.GetSeqView(read_start),
       seq_end = seq_pkg_.GetSeqView(read_end);
  auto seq_start_address = seq_start.raw_address(),
       seq_end_address = seq_end.raw_address();
  std::cout << "[ZMAC_DEBUG] NDP sequence from " << seq_start_address.first << " to " << seq_end_address.first << std::endl;
  SimNdpRangeGlobal(reinterpret_cast<std::uintptr_t>(seq_start_address.first),
                    reinterpret_cast<std::uintptr_t>(seq_end_address.first));
...
#ifdef SW_OPT
  // [ZMAC] Data structures for k-mer compression
  std::vector<std::pair<int64_t, unsigned>> kmer_buffer;
  unsigned buffer_limit = 32;
#endif
...
#ifdef SW_OPT
    // [ZMAC] Software implementation for k-mer compression
    uint64_t seq_id = seq_pkg_.GetSeqID(full_offset >> 1u);
    unsigned source_thread = seq_id * _n_threads / seq_pkg_.seq_count();     
    //std::cout << "[ZMAC Debug] add a kmer-pair (" << full_offset << ", " << source_thread 
    //          << " = " << seq_id << " * " << _n_threads << " / " << seq_pkg_.seq_count() 
    //          << ") to the kmer buffer (" << kmer_buffer.size() << ")" << std::endl; 

    if (kmer_buffer.size() >= buffer_limit) {
        int max_count = 0, max_thread = -1;
        std::map<int, int> thread_count;
        for (auto kmer_pair : kmer_buffer) {
            thread_count[kmer_pair.second]++;
            if (thread_count[kmer_pair.second] > max_count) {
                max_count = thread_count[kmer_pair.second];
                max_thread = kmer_pair.second;
            }
        }
        unsigned compressed_kmer_len = opt_.k + 1;
        int64_t cur_address = -1;
        for (auto kmer_pair : kmer_buffer) {
            if (int(kmer_pair.second) == max_thread) {
                if (cur_address == -1) {
                    cur_address = kmer_pair.first;
                } else {
                    if (kmer_pair.first > cur_address && kmer_pair.first < cur_address + opt_.k) {
                        compressed_kmer_len++;
                    } else {
                        compressed_kmer_len += opt_.k;
                    }
                    cur_address = kmer_pair.first;
                }
            }
        }
        kmer_buffer.erase(std::remove_if(kmer_buffer.begin(), kmer_buffer.end(), 
                                         [max_thread](std::pair<int64_t, int> v){return v.second == max_thread;}),
                          kmer_buffer.end());
        //std::cout << "[ZMAC Debug] remove kmers for thread-" << max_thread << " - #removed kmers = " << buffer_limit + 1 - kmer_buffer.size() << std::endl;
    }
    kmer_buffer.push_back(std::make_pair(full_offset, source_thread));
#endif
