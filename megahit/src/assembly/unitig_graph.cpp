...
UnitigGraph::UnitigGraph(SDBG *sdbg)
  ...
  // [Sniper] declare NDP data
  SimRoiStart();
  auto lock_start_addr = locks.get_begin(),
       lock_end_addr = locks.get_end();
  unsigned n_threads = omp_get_max_threads();
  SimNdpRangeGlobal(lock_start_addr, lock_end_addr);
  sdbg->declareNdpData(n_threads);
...
#ifdef SW_OPT
    std::vector<int> speculation;    
    int path_length = 0;
    unsigned buffer_limit = n_threads;
#endif
...
#ifdef SW_OPT
        speculation.push_back(path_length++);    
        if (speculation.size() >= buffer_limit) {
            speculation.erase(speculation.begin());
        }
        int min = 99999;
        for (auto v : speculation) {
            if (v < min) min = v;
        }
#endif
      }
