...
class SDBG {
 public:
  ...
  // [NDP] declare NDP data for SDBG
  void declareSdbgNdp(SdbgRawContent* raw_content, unsigned n_threads) {
      // get small_mul_size addresses
      auto small_mul_size = raw_content->small_mul.size();
      small_mul_t* small_mul_start = raw_content->small_mul.data(),
                   *small_mul_end = &(raw_content->small_mul[small_mul_size-1]);
      std::cout << "[ZMAC DEBUG] declare NDP data in assemble for #threads=" << n_threads << std::endl;
  
      // declare w vector
      SimNdpRangeGlobal(reinterpret_cast<std::uintptr_t>(raw_content->w.data()), 
                        reinterpret_cast<std::uintptr_t>(raw_content->w.data()+raw_content->w.word_count()));
      // declare last vector
      SimNdpRangeGlobal(reinterpret_cast<std::uintptr_t>(raw_content->last.data()), 
                        reinterpret_cast<std::uintptr_t>(raw_content->last.data()+raw_content->last.word_count()));
      // declare tip vector
      SimNdpRangeGlobal(reinterpret_cast<std::uintptr_t>(raw_content->tip.data()), 
                        reinterpret_cast<std::uintptr_t>(raw_content->tip.data()+raw_content->tip.word_count()));
      // declare small_mul vector
      SimNdpRangeGlobal(reinterpret_cast<std::uintptr_t>(small_mul_start), 
                        reinterpret_cast<std::uintptr_t>(small_mul_end));
  }

  void declareNdpData(unsigned n_threads) {
      declareSdbgNdp(&content_, n_threads);
  }
