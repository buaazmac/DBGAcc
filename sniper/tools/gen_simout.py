...
def generate_simout(jobid = None, resultsdir = None, partial = None, output = sys.stdout, silent = False):
  ...
  # [ZMAC] Add PIM offloading statistics
  results['performance_model.cycle_count_fixed_pim'] = [
    results['performance_model.elapsed_time_pim'][c] * results['fs_to_cycles_cores'][c]
    for c in range(ncores)
  ]

  results['interval_timer.overall_time'] = [
    results['interval_timer.instExeLatency'][c] \
    + results['interval_timer.instLoadLatency'][c] \
    + results['interval_timer.instStoreLatency'][c]
    for c in range(ncores)
  ]
  results['interval_timer.pim_time'] = [
    results['interval_timer.instExeLatencyPim'][c] \
    + results['interval_timer.instLoadLatencyPim'][c] \
    + results['interval_timer.instStoreLatencyPim'][c]
    for c in range(ncores)
  ]
  results['interval_timer.overall_cp_time'] = [
    results['interval_timer.instExeCpLatency'][c] \
    + results['interval_timer.instLoadCpLatency'][c] \
    + results['interval_timer.instStoreCpLatency'][c]
    for c in range(ncores)
  ]
  results['interval_timer.pim_cp_time'] = [
    results['interval_timer.instExeCpLatencyPim'][c] \
    + results['interval_timer.instLoadCpLatencyPim'][c] \
    + results['interval_timer.instStoreCpLatencyPim'][c]
    for c in range(ncores)
  ]
  results['performance_model.lll_cycle'] = [
    results['performance_model.lll_time'][c] * results['fs_to_cycles_cores'][c]
    for c in range(ncores)
  ]
  results['performance_model.lll_cycle_pim'] = [
    results['performance_model.lll_time_pim'][c] * results['fs_to_cycles_cores'][c]
    for c in range(ncores)
  ]
  template += [
    ('PIM Offloading Summary', '', ''),
    ('  Instructions', 'performance_model.instruction_count_pim', str),
    ('  Cycles',       'performance_model.cycle_count_fixed_pim', format_int),
    ('  Time (ns)',    'performance_model.elapsed_time_pim', format_ns(0)),
    ('  Idle time (ns)', 'performance_model.idle_elapsed_time_pim', format_ns(0)),
    ('  Idle time (ns)', 'performance_model.idle_elapsed_time_pim', format_ns(0)),
    ('  Long Latency Load time (ns)', 'performance_model.lll_time', format_ns(0)),
    ('  Long Latency Load cycle', 'performance_model.lll_cycle', format_int),
    ('  Long Latency Load time (ns) (PIM)', 'performance_model.lll_time_pim', format_ns(0)),
    ('  Long Latency Load cycle (PIM)', 'performance_model.lll_cycle_pim', format_int),
    ('  Overall Cycle', 'interval_timer.overall_time', format_int),
    ('     InstExe Cycle', 'interval_timer.instExeLatency', format_int),
    ('     InstLoad Cycle', 'interval_timer.instLoadLatency', format_int),
    ('     InstStore Cycle', 'interval_timer.instStoreLatency', format_int),
    ('  PIM Offloading Cycle', 'interval_timer.pim_time', format_int),
    ('     InstExePIM Cycle', 'interval_timer.instExeLatencyPim', format_int),
    ('     InstLoadPIM Cycle', 'interval_timer.instLoadLatencyPim', format_int),
    ('     InstStorePIM Cycle', 'interval_timer.instStoreLatencyPim', format_int),
    ('  Overall Critical Cycle', 'interval_timer.overall_cp_time', format_int),
    ('     InstExeCp Cycle', 'interval_timer.instExeCpLatency', format_int),
    ('     InstLoadCp Cycle', 'interval_timer.instLoadCpLatency', format_int),
    ('     InstStoreCp Cycle', 'interval_timer.instStoreCpLatency', format_int),
    ('  PIM Offloading Critical Cycle', 'interval_timer.pim_cp_time', format_int),
