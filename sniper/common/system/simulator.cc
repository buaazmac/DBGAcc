...
//   m_transport->barrier();
   ...
   /*[ZMAC] create a ofstream for memory trace */
   memtrace_ofs.open(m_memtrace_file.c_str(), std::ofstream::out | std::ofstream::app);
}
...
Simulator::~Simulator()
{
   ...
   /*[ZMAC] close the ofstream for memory trace */
   memtrace_ofs.close();
}
