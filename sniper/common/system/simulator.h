...
public:
   ...
   // [ZMAC] functions for CPU-PIM simulation
   ...
   void startPimOffload() {
       //std::cout << "[ZMAC] Start PIM offloading session!\n";
       if (!m_using_cpu) {
           std::cout << "Error: the cpu has already been disabled!\n";
           return;
       }
       m_using_cpu = false;
       m_using_pim = true;
   ...
   void endPimOffload() {
       //std::cout << "[ZMAC] End PIM offloading session!\n";
       if (m_using_cpu) {
           std::cout << "Error: the cpu is being used!\n";
           return;
       }
       m_using_cpu = true;
       m_using_pim = false;
   ...
   void recordMemoryTrace(std::string trace) {
#ifdef MEM_TRACE_DEBUG
       memtrace_ofs << trace;
#endif
   }
...
private:
   ...
   // [ZMAC] parameters for CPU-PIM simulation
   bool m_using_cpu = true, m_using_pim = false;
   std::string  m_memtrace_file = "mem.trace";
   std::ofstream memtrace_ofs;
