...
PerformanceModel::PerformanceModel(Core *core)
   ...
   , m_instruction_count_pim(0)
   , m_elapsed_time_pim(Sim()->getDvfsManager()->getCoreDomain(core->getId()))
   , m_idle_elapsed_time_pim(Sim()->getDvfsManager()->getCoreDomain(core->getId()))
   , m_lll_time(Sim()->getDvfsManager()->getCoreDomain(core->getId()))
   , m_lll_time_pim(Sim()->getDvfsManager()->getCoreDomain(core->getId()))
   ...
   , m_current_ins_index(0)
{
   ...
   /* [ZMAC] register PIM offloading statistics */
   registerStatsMetric("performance_model", core->getId(), "instruction_count_pim", &m_instruction_count_pim);
   registerStatsMetric("performance_model", core->getId(), "elapsed_time_pim", &m_elapsed_time_pim);
   registerStatsMetric("performance_model", core->getId(), "lll_time", &m_lll_time);
   registerStatsMetric("performance_model", core->getId(), "lll_time_pim", &m_lll_time_pim);
   registerStatsMetric("performance_model", core->getId(), "idle_elapsed_time_pim", &m_idle_elapsed_time_pim);
...
void PerformanceModel::iterate()
{
   while (m_instruction_queue.size() > 0)
   {
      ...
      /*TODO: [ZMAC] Here we can collect instructions that are not simulated by detailed model. */
      //if (m_pim_sim && m_fastforward && !m_enabled) {
      //    // TODO: add the decoder/simulation codes
      //}
...
void PerformanceModel::incrementIdleElapsedTime(SubsecondTime time)
{
   ...
   // [ZMAC] update idle elapsed time for PIM offloading
   if (Sim()->isUsingPIM()) {
       m_idle_elapsed_time_pim.addLatency(time);
   }
