      ...
      , m_frequency_domain(core->getDvfsDomain())
{
   ...
   /* [ZMAC] register statistics for PIM offloading */
   // Total
   m_exe_latency = 0;
   m_load_latency = 0;
   m_store_latency = 0;
   m_exe_cp_latency = 0;
   m_load_cp_latency = 0;
   m_store_cp_latency = 0;
   registerStatsMetric("interval_timer", core->getId(), "instExeLatency", &m_exe_latency);
   registerStatsMetric("interval_timer", core->getId(), "instLoadLatency", &m_load_latency);
   registerStatsMetric("interval_timer", core->getId(), "instStoreLatency", &m_store_latency);
   registerStatsMetric("interval_timer", core->getId(), "instExeCpLatency", &m_exe_cp_latency);
   registerStatsMetric("interval_timer", core->getId(), "instLoadCpLatency", &m_load_cp_latency);
   registerStatsMetric("interval_timer", core->getId(), "instStoreCpLatency", &m_store_cp_latency);
   // PIM offloading
   m_exe_latency_pim = 0;
   m_load_latency_pim = 0;
   m_store_latency_pim = 0;
   m_exe_cp_latency_pim = 0;
   m_load_cp_latency_pim = 0;
   m_store_cp_latency_pim = 0;
   registerStatsMetric("interval_timer", core->getId(), "instExeLatencyPim", &m_exe_latency_pim);
   registerStatsMetric("interval_timer", core->getId(), "instLoadLatencyPim", &m_load_latency_pim);
   registerStatsMetric("interval_timer", core->getId(), "instStoreLatencyPim", &m_store_latency_pim);
   registerStatsMetric("interval_timer", core->getId(), "instExeCpLatencyPim", &m_exe_cp_latency_pim);
   registerStatsMetric("interval_timer", core->getId(), "instLoadCpLatencyPim", &m_load_cp_latency_pim);
   registerStatsMetric("interval_timer", core->getId(), "instStoreCpLatencyPim", &m_store_cp_latency_pim);
}
...
# endif
   ...
   if (latency == 0)
   {
      ...
      // [ZMAC] add 1 cycle for 0-latency window
       m_exe_latency += latency;
       m_exe_cp_latency += latency;
       if (Sim()->isUsingPIM()) {
           m_exe_latency_pim += latency;
           m_exe_cp_latency_pim += latency;
       }
   }
...
uint64_t IntervalTimer::dispatchInstruction(Windows::WindowEntry& micro_op, StopDispatchReason& continue_dispatching)
{
   ...
   /* [ZMAC] record the critical path latency that seems not to be counted in updateCriticalPathTail() */
   uint64_t implicit_cp_latency = 0;
...
# endif
            ...
            // [ZMAC] long latency load increases the critical path latency
            implicit_cp_latency += long_latency_load_latency;
         }
         else
         {
   ...
   /* [ZMAC] here we record the instruction exe/mem latency 
    *        and PIM offloading part */ 
   uint64_t cp_latency = micro_op.getCpContr() + implicit_cp_latency;
   if (micro_op.getMicroOp()->isExecute()) {
       m_exe_latency += latency;
       m_exe_cp_latency += cp_latency;
       if (Sim()->isUsingPIM()) {
           m_exe_latency_pim += latency;
           m_exe_cp_latency_pim += cp_latency;
       }
   } else if (micro_op.getMicroOp()->isLoad()) {
       m_load_latency += latency;
       m_load_cp_latency += cp_latency;
       if (Sim()->isUsingPIM()) {
           m_load_latency_pim += latency;
           m_load_cp_latency_pim += cp_latency;
       }
   } else if (micro_op.getMicroOp()->isStore()) {
       m_store_latency += latency;
       m_store_cp_latency += cp_latency;
       if (Sim()->isUsingPIM()) {
           m_store_latency_pim += latency;
           m_store_cp_latency_pim += cp_latency;
       }
   }
