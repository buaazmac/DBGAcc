...
void MicroOpPerformanceModel::handleInstruction(DynamicInstruction *dynins)
{
   ...
   else if (insn_cost > SubsecondTime::Zero())
   {
      ...
      {
         // [ZMAC] Here we add long latency load cycle for checking statistics correctness
         m_lll_time.addLatency(cost_add_latency_now);
         if (Sim()->isUsingPIM()) {
             m_lll_time_pim.addLatency(cost_add_latency_now);
         }
   ...
   /* TODO: [ZMAC] Here we can record CPU ins/mem time & PIM ins/mem time */
   if (Sim()->isUsingPIM()) {
       m_instruction_count_pim += new_num_insns;
